...
def get_free_tables(tables):
    ...
    free_tables = []
    for table in tables:
        if not table["occupied"]:  # occupied == F
            free_tables.append(table["table_id"])
    return free_tables


def find_two_table_for_size(tables, party_size):
    """
    Level 2
...
if __name__ == "__main__":
    ...
    tables_data = [
        ...
        {"table_id": 3, "capacity": 2, "occupied": True, "neighbors": [2, 4]},
    ...
    print("LEVEL 2: One table for party size 4 =", find_two_table_for_size(tables_data, 2))

    print("LEVEL 3: tables 2 and 4 for party size 4 =", find_all_tables_for_size(tables_data, 2))
